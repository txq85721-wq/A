...
from .strategies.risk_filters import has_enough_history, is_new_stock, should_skip_by_meta
...
def run_daily_pipeline() -> dict:
    ...
    for row in universe:
        ...
        try:
            ...
            if not has_enough_history(history, settings.min_history_days):
                skipped += 1
                failure_reasons["not_enough_history"] += 1
                continue
            if is_new_stock(history, settings.min_history_days):
                skipped += 1
                failure_reasons["new_stock"] += 1
                continue
            ...
            if stock_candidates:
                ...
                for candidate in stock_candidates:
                    ...
                    payload["recent_news"] = [item.get("title", "") for item in related_news if item.get("title")]
                    candidates.append(candidate.__class__(**{k: v for k, v in payload.items() if k in candidate.to_dict()}))
                    candidates[-1].metrics["recent_news"] = payload["recent_news"]
    ...
    for item in deduped[:30]:
        ...
        if not payload.get("recent_news"):
